...
def get_parser():
    ...
    parser.add_argument('--clip', type=arg_boolean, default=True)
...
class Processor():
    ...
    def train(self, epoch, save_model=True):
        ...
        if (not arg.accumulating_gradients):
            for batch_idx, (data, label, name) in enumerate(loader):
                ...
                np.save('' + name_exp + '/conf_train_' + str(epoch),
                        conf_matrix_train)
    ...
    def test(self, epoch, save_score=True, loader_name=['test']):
        ...
        val_total = 0
        conf_matrix_test = 0
        class_correct = list(0. for i in range(0, 60))
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, name) in enumerate(self.data_loader[ln]):
                ...
                conf_matrix_test += confusion_matrix(predictions.cpu(), label.cpu(), labels=np.arange(60))
    ...
    def val(self, epoch, save_score=False, loader_name=['val']):
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label) in enumerate(self.data_loader[ln]):
                ...
                conf_matrix_val += confusion_matrix(predictions.cpu(), label.cpu(), labels=np.arange(60))
                np.save("" + name_exp + "/conf_val_" + str(epoch),
                        conf_matrix_val)
    ...
    def start(self):
        ...
        if not self.arg.training:
            self.test(epoch=0, save_score=True, loader_name=['test'])
        patience = 50
        patient_counter = 0
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if eval_model and self.arg.validation_split:
                    accuracy = self.val(
                        epoch,
            ...
            self.load_checkpoint(self.arg.log_dir, "epoch%d.ckpt" % self.best_epoch)
            self.test(
                epoch=0, save_score=self.arg.save_score, loader_name=['test'])
            self.print_log('Done.\n')
